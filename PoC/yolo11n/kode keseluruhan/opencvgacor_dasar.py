...
B_SHIFT = 0       # fine-tune blue-yellow balance
OMEGA = 0.75       # dehazing strength
CLAHE_CLIP = 1.2   # CLAHE clip limit
RED_STRENGTH = 30  # red channel restoration intensity
T_MIN = 0.35       # minimum transmission for dehazing
...
# ------------------------------
# Contoh penggunaan
img_path = r"D:\KULIAH ITB Daffa\SEM 7\PERTAAN\peryoloan\model\model_datasetfinal3\testing_opencvgacor\gb26.png"
save_folder = r"D:\KULIAH ITB Daffa\SEM 7\PERTAAN\peryoloan\model\model_datasetfinal3\testing_opencvgacor"

process_and_save(img_path, save_folder)
"""
...
"""
========================================================================
INI KODE UNTUK YANG GB NYA BANYAK
def process_folder(input_folder, save_folder, exts=(".png", ".jpg", ".jpeg")):
    os.makedirs(save_folder, exist_ok=True)
    ...
    for fname in sorted(os.listdir(input_folder)):
        if fname.lower().endswith(exts):
            ...
            enhanced = enhance_underwater(img)
            save_path = os.path.join(save_folder, f"enhanced_{fname}")
            cv2.imwrite(save_path, enhanced)
